...
export const patientService = {
  ...
  getPatientConsultation: async (patientId, consultationId) => {
    const response = await api.get(`/patients/${patientId}/consultation/${consultationId}`);
    return response.data;
  },

  // Get all active patients in organization
  getOrganizationPatients: async () => {
    const response = await api.get('/patients/organization');
    return response.data;
  },
  ...
  createPatientWithDocuments: async (patientData, medicalFiles = []) => {
    ...
    // Generate a proper UUID if one isn't provided
    const patientUuid = patientData.patient_uuid || crypto.randomUUID();
    ...
    const response = await fetch(`${AI_BACKEND_URL}/patients/${patientUuid}/documents/`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.detail || 'Failed to upload patient documents');
    }

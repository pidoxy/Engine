...
import { LanguageCode, NaijaMessage, RecordingState } from '@/lib/triage/types';
import { LANGUAGES } from '@/lib/triage/languages';
import { continueConversation, processText, parseTriageResult } from '@/lib/triage/api';
import { speakText, stopCurrentAudio } from '@/lib/triage/tts';
import SpeakButton from '@/components/triage/SpeakButton';
import type { NaijaTriageResult } from '@/lib/triage/types';
...
export default function NaijaConversation({ language, onCancel, onComplete }: NaijaConversationProps) {
  ...
  const sendAudioMessage = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1/naija/process_audio/`, {
        method: 'POST',
        body: formData,
      });
